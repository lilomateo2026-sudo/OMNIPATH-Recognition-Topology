...
def classify_fixture(fixture: dict[str, Any]) -> list[str]:
    ...
    # Missing lineage is provenance loss; present-but-wrong lineage is corruption.
    ...
    if (
        ...
        # Wrong explicit ancestry is corruption, not merely absence.
    ...
    # Agreement is evidence only; transforms/judgments that promote it to proof are pathological.
    if (
    ...
    # Observer output cannot be recycled as raw/independent evidence.
    observed_sources = [
    ...
    # Explicit evidence-dropping transforms with an empty contradiction ledger are invalid.
    if (
    ...
    # Hash identity has priority over a self-asserted replay match flag.
